...
def loss(logits, labels, num_classes, head=None):
    ...
    with tf.name_scope('loss'):
        ...
        labels = tf.compat.v1.to_float(tf.reshape(labels, (-1, num_classes)))

        softmax = tf.nn.softmax(logits) + epsilon

        if head is not None:
            cross_entropy = -tf.compat.v1.reduce_sum(tf.compat.v1.multiply(labels * tf.compat.v1.log(softmax),
